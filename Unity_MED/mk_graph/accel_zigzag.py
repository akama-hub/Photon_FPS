...
with open(f'/mnt/HDD/akama/Unity/mlp/accel/zigzag/tp_xyv_xy/diff_result.csv') as f:
    reader = csv.reader(f)
    for row in reader:
...
with open(f'/mnt/HDD/akama/Unity/2mlp/accel/zigzag/TP_xyV_xy/AccelEvaluate/diff_result.csv') as f:
    reader = csv.reader(f)
    for row in reader:
...
plt.savefig(f"./figure/zigzag/accel_zigzag.eps", bbox_inches='tight', pad_inches=0)
plt.savefig(f"./figure/zigzag/accel_zigzag.png", bbox_inches='tight', pad_inches=0)
plt.clf()
plt.close()