...
# with open(f'/mnt/HDD/akama/Unity/2mlp/Fixed/ohuku/TP_xy/speed5/2action/evaluate/diff_result.csv') as f:
with open(f'/mnt/HDD/akama/Unity/2mlp/Fixed/ohuku/TP_xy/2action/evaluate/diff_result.csv') as f:
    reader = csv.reader(f)
    for row in reader:
...
for frame_delay in range(1, 11):
    ...
    with open(f"/mnt/HDD/akama/Unity/LR-NLR/ohuku/{frame_delay}frame/result.csv") as f:
        reader = csv.reader(f)
        for row in reader:
...
plt.savefig(f"./figure/ohuku/2action_ohuku.eps", bbox_inches='tight', pad_inches=0)
plt.savefig(f"./figure/ohuku/2action_ohuku.png", bbox_inches='tight', pad_inches=0)
plt.clf()
plt.close()