...
# with open(f'/mnt/HDD/akama/Unity/2mlp/curb/TP_xy/speed5/evaluate/diff_result.csv') as f:
with open(f'/mnt/HDD/akama/Unity/2mlp/Fixed/curb/TP_xy/evaluate/diff_result.csv') as f:
    reader = csv.reader(f)
    for row in reader:
...
for frame_delay in range(1, 11):
    ...
    with open(f"/mnt/HDD/akama/Unity/LR-NLR/curb/{frame_delay}frame/result.csv") as f:
        reader = csv.reader(f)
        for row in reader:
            if frame_delay == 1:
                ...
                delay_diff.append(float(row[2]))
                lr_diff.append(float(row[5].replace("[", "").replace("]", "")))
                nlr_diff.append(float(row[8]))
...
# plt.savefig(f"./figure/curb/speed5_curb.eps", bbox_inches='tight', pad_inches=0)
# plt.savefig(f"./figure/curb/speed5_curb.png", bbox_inches='tight', pad_inches=0)
plt.savefig(f"./figure/curb/curb.eps", bbox_inches='tight', pad_inches=0)
plt.savefig(f"./figure/curb/curb.png", bbox_inches='tight', pad_inches=0)
plt.clf()
plt.close()