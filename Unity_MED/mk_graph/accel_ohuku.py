...
with open(f'/mnt/HDD/akama/Unity/mlp/accel/ohuku/tp_xyv_xy/diff_result.csv') as f:
    reader = csv.reader(f)
    for row in reader:
...
with open(f'/mnt/HDD/akama/Unity/2mlp/accel/ohuku/TP_xyV_xy/9action/Accelevaluate/diff_result.csv') as f:
    reader = csv.reader(f)
    for row in reader:
...
plt.savefig(f"./figure/ohuku/accel_9action_ohuku.eps", bbox_inches='tight', pad_inches=0)
plt.savefig(f"./figure/ohuku/accel_9action_ohuku.png", bbox_inches='tight', pad_inches=0)
plt.clf()
plt.close()