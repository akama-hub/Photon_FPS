...
def main():
    ...
    log_dir = f'/mnt/HDD/Photon_FPS/DRLModels/NAF/{motion}/{log_date}'
    ...
    change_q_func1 = FCQuadraticStateQFunction(n_dim_obs, action_size, n_hidden_channels, n_hidden_layers, action_space, True)
    change_q_func2 = FCQuadraticStateQFunction(n_dim_obs, action_size, n_hidden_channels, n_hidden_layers, action_space, True)
    """
    Args:
    ...
    # Noisy nets
    pnn.to_factorized_noisy(change_q_func1, sigma_scale=0.5)
    pnn.to_factorized_noisy(change_q_func2, sigma_scale=0.5)
    pnn.to_factorized_noisy(act_q_func, sigma_scale=0.5)
    
    # Turn off explorer
    explorer = explorers.Greedy()

    change_opt1 = torch.optim.Adam(change_q_func1.parameters(), 6.25e-5, eps=1.5 * 10 ** -4)
    change_opt2 = torch.optim.Adam(change_q_func2.parameters(), 6.25e-5, eps=1.5 * 10 ** -4)
