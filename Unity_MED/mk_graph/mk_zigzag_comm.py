...
for sn in range(0, 21, 5):
    diff_position = []
    with open(f'/mnt/HDD/akama//Unity/2mlp/zigzag/TP_xy/comm/speed5/digital/sn{sn}/diff_result.csv') as f:
        reader = csv.reader(f)
        for row in reader:
    ...
    diff_position = []
    with open(f'/mnt/HDD/akama//Unity/2mlp/zigzag/TP_xy/comm/speed5/analog/sn{sn}/diff_result.csv') as f:
        reader = csv.reader(f)
        for row in reader:
            diff_position.append(float(row[6]))
...
for frame_delay in range(1, 11):
    ...
    plt.savefig(f"./figure/zigzag/zigzag_comm_delay{frame_delay}F.eps", bbox_inches='tight', pad_inches=0)
    plt.savefig(f"./figure/zigzag/zigzag_comm_delay{frame_delay}F.png", bbox_inches='tight', pad_inches=0)
    plt.clf()
    plt.close()