    ...
    while True:
        try:
            ...
            if cnt < 4:
                ...
                velocity = [vel_x[0], vel_y[0]]
                last_accel = accel
                accel = [(pos_x[0] - pos_x[1])/(t[0]- t[1]), (pos_y[0] - pos_y[1])/(t[0]- t[1])]
                ...
                if change_second == 0:
                    if last_action == action:
                        predict_x, predict_y = MAADR(position, velocity, accel, last_accel, lag)
                    else:
                        
                elif change_second > lag:
                    predict_x, predict_y = MAADR(position, velocity, accel, last_accel, lag)
